...
def translate_text(source_text: str, source_language: str, target_language: str) -> str:
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            {
                "role": "system",
                "content": f"You are an expert in pharmacy. In the following you get an original {source_language} text from a Java properties file and several translations into other languages."
                           f"Translage the original English text into {target_language}. Ensure that the translated text retains the original meaning, tone, and intent."
...
def read_source_properties(abs_path: str) -> dict:
    props: dict = read_properties(abs_path=abs_path)
